...
%      TRACEFP, by itself, creates a new TRACEFP or raises the existing
%      singleton*.
%
%      H = TRACEFP returns the handle to a new TRACEFP or the handle to
%      the existing singleton*.
%
%      TRACEFP('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in TRACEFP.M with the given input arguments.
%
%      TRACEFP('Property','Value',...) creates a new TRACEFP or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before TraceFP_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to TraceFP_OpeningFcn via varargin.
%
...
handles.control_points = zeros(0,2); % no control points (each row (x,y))
handles.triangles = zeros(0,3); % no polygons yet, each indexes 3 ctrl pts
...
% Hint: get(hObject,'Value') returns toggle state of show_wall_samples
...
% Hint: get(hObject,'Value') returns toggle state of show_control_points
...
orient = det([ 	(handles.control_points(pinds(1),:) ...
		...
		(handles.control_points(pinds(2),:) ...
			- handles.control_points(pinds(3),:)) ]);
if(orient < 0)
	fprintf('[TraceFP]\t\treordering to be counter-clockwise\n');
	pinds = fliplr(pinds);
end
...
if(ind <= 0)
	...
	% make current room index to be the room of the selected triangle
...
% open the file
[fpfile, pathname,success] = uigetfile('*.fp','Open Existing Floorplan');
if(~success)
	fprintf('[TraceFP]\t\tNevermind.\n');
	return;
end
confirm = questdlg(['Loading a floorplan will erase any existing data. ',...
