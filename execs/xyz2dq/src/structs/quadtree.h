...
class quadtree_t
{
	...
	/* insert:
	 *
	 *	Inserts a point with pose and height information.
	 *
	 * return value:
	 *
	 * 	Returns the appropriate data on success.
	 * 	Returns NULL on error.
	 */
	quaddata_t* insert(point_t& p, unsigned int pose_ind, double z);
	 ...
	 * arguments:
	 *
	 * 	os -	Where to print to
	 * 	n -	wall sample point threshold 
	 * 	h -	Wall sample height threshold
	 *
	 * format:
	 *
	 * 	<max_depth>
	 ...
	 * 	a quaddata_t struct.
	 */
	void print(ostream& os, int n, double h);
...
class quaddata_t
{
	...
	set<int> height_bins; /* each integer represents a discretized
				height value that was seen by this
				leaf */
	normal_t norm;
	...
	/* has_wall_coverage:
	 * 
	 * 	Returns true if the contained samples
	 * 	span a sufficient vertical extent to be
	 * 	considered a "wall".
	 *
	 * arguments:
	 *
	 * 	n -	Minimum number of wall samples required
	 * 	h -	Wall height threshold
	 */
	bool has_wall_coverage(int n, double h);
	 ...
	 * arguments:
	 *
	 * 	os -	Where to print the info
	 * 	n -	The minimum number of points for a cell to 
	 * 		be printed
	 * 	h -	The minimum height threshold to use
	 *
	 * format:
	 *
