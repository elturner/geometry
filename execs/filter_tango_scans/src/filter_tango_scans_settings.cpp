...
int filter_tango_scans_run_settings_t::parse(int argc, char** argv)
{
	...
	args.set_program_description("This program converts .dat files "
			...
			"file.");
	args.add(SENSOR_NAME_FLAG, "Specifies the name of the tango depth "
			"sensor to filter in this program.", false, 1);
	...
	args.add_required_file_type(FILE_EXT_XML, 1,
			"The input time synchronization .xml file, in order "
			"to convert timestamps from tango sensor clock to "
			"the system common clock.");
	...
	/* retrieve specified timestamp file */
	files.clear(); args.files_of_type(FILE_EXT_XML, files);
	if(files.size() > 1)
	{
		cerr << "[filter_tango_scans_run_settings::parse]\t"
		     << "Multiple input timesync (.xml) files given, "
		     << "only the first will be parsed." << endl;
	}
	this->timefile = files[0];
	...
	this->sensor_name = args.get_val(SENSOR_NAME_FLAG);
