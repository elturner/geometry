...
quaddata_t* quadtree_t::insert(point_t& p)
{
	...
	if(ret == NULL)
	{
		cerr << "[insert]\tError inserting point into tree" 
		     << endl;
		cerr << "\t\tp = ";
		p.print(cerr);
		cerr << endl;
...
quaddata_t* quadtree_t::insert(point_t& p, unsigned int pose_ind, double z)
{
	quaddata_t* dat;
	...
	/* add height */
	dat->height_bins.insert((int) (z / VERTICAL_BIN_SIZE));
	return dat;
}
...
void quadtree_t::print(ostream& os, int n, double h)
{
	/* set up stream */
...
void quadnode_t::print(ostream& os, int n, double h)
{
	int i;
...
quaddata_t* quaddata_t::clone()
{
	...
	c->pose_inds.insert(this->pose_inds.begin(), 
				this->pose_inds.end());
	c->height_bins.insert(this->height_bins.begin(),
				this->height_bins.end());
	c->norm = this->norm;
...
bool quaddata_t::has_wall_coverage(int n, double h)
{
	return (this->num_points >= n)
		&& (this->height_bins.size() >= (h / VERTICAL_BIN_SIZE));
}
	
void quaddata_t::print(ostream& os, int n, double h)
{
	set<unsigned int>::iterator it;

	/* only print data with wall coverage */
	if(!(this->has_wall_coverage(n, h)))
		return;
	...
	/* range of heights (min,max) */
	os << ((*(this->height_bins.begin())) * VERTICAL_BIN_SIZE) << " ";
	os << ((*(this->height_bins.rbegin())) * VERTICAL_BIN_SIZE) << " ";
	...
	/* write poses */
	for(it = this->pose_inds.begin(); it != this->pose_inds.end(); 
						it++)
		os << " " << (*it);
