/*
*	magnetometerProp.cpp
*
*	This class acts as a container for all the magnetometer properties
*	that are passed in via the xml config file
*/

#include "magnetometerProp.h"
...
*/
magnetometerProp::magnetometerProp() {
	this->name = "";
	this->configFile = "";
}
 ...
 *		1 - Missing Property
 */
int magnetometerProp::assign_props(std::map<std::string,std::string>& property_map) {
	...
	// Extract the ucontroller file name
	it = property_map.find("ucontroller");
	if(it == property_map.end()) {
		ret_code = 1;
	}
	else {
		this->ucontroller = it->second;
	}
...
* laser type objects.
*/
std::string magnetometerProp::type_tag() const {
